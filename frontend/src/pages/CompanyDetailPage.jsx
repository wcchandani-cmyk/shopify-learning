...
import EditCompanyDetailsModal from "../components/company/EditCompanyDetailsModal";
import AddCustomerToCompanyModal from "../components/company/AddCustomerToCompanyModal";
import ChangeMainContactModal from "../components/company/ChangeMainContactModal";
import RemoveCustomerModal from "../components/company/RemoveCustomerModal";
import EditAssignedStaffModal from "../components/company/EditAssignedStaffModal";
import ManagePermissionsModal from "../components/customers/ManagePermissionsModal";
...
export default function CompanyDetailPage() {
  ...
  // Only show the full-page error/loader before we have any data. Once the
  // company is loaded, refreshes (e.g. after a modal save) keep the page — and
  // its mounted modals — in the tree so an open overlay is never torn out.
  if (error && !company) {
    return (
  ...
  return (
    <s-page heading={company.name}>
      ...
      {/* Main Two-Column Layout using Polaris Container Queries */}
      <s-query-container containerName="product-detail">
        <div className="product-detail-layout">
          
          {/* Left Column (Main Actions & Locations) */}
          <div className="product-detail-layout__main">
            <s-stack gap="base">
              
              {/* Not approved with no customer: prompt to add a customer */}
              ...
              {/* Not approved with a customer: prompt to grant ordering permission */}
              {!isApproved && firstContact && (
              ...
              {/* Orders summary when ordering is approved */}
              {isApproved && (
                <s-section>
                  {totalOrders === 0 ? (
                    <s-stack gap="base">
              ...
              {/* Locations Section inside s-section */}
              <s-section>
                <s-stack gap="base">
                  <s-stack
                    direction="inline"
          ...
          {/* Right Column (Company details, Customizations, Notes) */}
          <div className="product-detail-layout__aside">
            <s-stack gap="base">
              
              {/* Card 1: Company Profile & Customers */}
              <s-section>
                <s-stack gap="base">
                  <s-stack
                    direction="inline"
